...
func TestApplyGenerationDefaults(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "generation_config.json"), []byte(`{"max_new_tokens":32,"temperature":0.7,"top_k":20,"top_p":0.8,"repetition_penalty":1.1,"stop_strings":["</s>"]}`), 0o644); err != nil {
		t.Fatal(err)
	}
	req := GenerateRequest{}
	...
	if req.MaxTokens != 32 || req.Temperature != 0.7 || req.TopK != 20 || req.TopP != 0.8 || req.RepeatPenalty != 1.1 || len(req.Stop) != 1 || req.Stop[0] != "</s>" {
...
func TestApplyGenerationDefaultsDoesNotOverrideExplicitValues(t *testing.T) {
	...
	req := GenerateRequest{MaxTokens: 4, Temperature: 0.2, TopK: 4, Stop: []string{"END"}}
	if err := ApplyGenerationDefaults(dir, &req); err != nil {
		t.Fatal(err)
	}
	if req.MaxTokens != 4 || req.Temperature != 0.2 || req.TopK != 4 || len(req.Stop) != 1 || req.Stop[0] != "END" {
		t.Fatalf("unexpected request: %#v", req)
	}
}
