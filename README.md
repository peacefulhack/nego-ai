...
Current status: it loads safetensors metadata, tensor readers, selected F32/F16/BF16 tensors as float32, HF Qwen/Llama weight manifests, tokenizer.json, native adapters, early float32 math primitives, prompt decode state, KV cache history, and guarded greedy generation for small compatible decoder fixtures. Full production-quality autoregressive generation for Qwen/Llama models is still in progress.
