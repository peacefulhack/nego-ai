...
8. Resolve local model artifacts into format, run backend, and training backend compatibility reports.
9. Tokenize text, count context usage, and render chat prompts with WordLevel, BPE, and Unigram/SentencePiece-style tokenizer metadata.
10. Run local GGUF chat through `llama.cpp` when `llama-cli` is installed.
11. Use remote OpenAI-compatible chat and embedding APIs.
12. Prepare, validate, convert, filter, split, and sample datasets.
13. Run eval suites and compare reports.
14. Create, validate, and run external training job JSON files.
15. Create local share manifests and archive packages for trained model output directories.
16. Upload regular files or small folders to Hub repos through inline commit uploads.
