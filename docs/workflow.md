...
`native` is the pure-Go runtime track. It can load GGUF metadata, tensor directories, raw tensor bytes, selected tensors as float32 buffers, and early CPU tensor math primitives today, but full transformer inference is still under development.
